...
uint8_t game_iter(void *data) {
    ...
    if (data_ptr->cursor_cooldown == 0) {
        if (joypad_current & J_UP) {
            ...
            if (data_ptr->board[data_ptr->cursor_x][data_ptr->cursor_y]) {
                // a piece exists here
                // if (data_ptr->move_counter % 2)
                if (data_ptr->cursor_target) {
                    data_ptr->cursor_target = NULL;
                } else {
                    data_ptr->cursor_target = &data_ptr->board[data_ptr->cursor_x][data_ptr->cursor_y];
                }
            }
        }
        if (data_ptr->cursor_target) {
            data_ptr->cursor_target->
        }
    } else {
